...
import biz.zhidu.zdsdk.VideoRenderer.I420Frame;
...
public class VideoStreamsView extends GLSurfaceView
    ...
    private Point remoteSrcDimensions = null;
    // [0] are local Y,U,V, [1] are remote Y,U,V.
    // private int[][] yuvTextures = { { -1, -1, -1}, {-1, -1, -1 }};
    private int[][] yuvTextures = {{-1, -1, -1, -1}, {-1, -1, -1, -1}};
    ...
    public VideoStreamsView(Context c, Point screenDimensions) {
        ...
        Log.i(TAG, "VideoStreamsView: " + screenDimensions.x + "x" + screenDimensions.y);
    }
    ...
    public void queueFrame(final Endpoint stream, I420Frame frame) {
        ...
        synchronized (framesToRender) {
            // A new render needs to be scheduled (via updateFrames()) if there isn't
            // already a render scheduled, which is true if framesToRender is empty.
    ...
    public void setSize(Endpoint stream, int width, int height) {
        ...
        checkNoGLES2Error();
        if (stream == Endpoint.REMOTE)
            remoteSrcDimensions = new Point(width, height);
    }
...
//    drawRectangle(yuvTextures[0], localVertices);
    ...
    public static Bitmap createTextBitmap(String text, int w, int h) {
        ...
        String[] mLines = text.split("\n");
        // draw the text centered
        int yPos = bounds.height() + 5;
        for (int i = 0; i < mLines.length; i++) {
            canvas.drawText(mLines[i], 5, yPos, textPaint);
